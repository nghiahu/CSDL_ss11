...
-- 3) Hãy tạo một VIEW có tên HighSalaryEmployees, 
-- chỉ hiển thị những nhân viên có mức lương từ 15 triệu trở lên. Khi cập nhật dữ liệu trong VIEW, dữ liệu mới cũng phải thỏa mãn điều kiện này.
...
-- 5) Cập nhật VIEW EmployeeBranch để thêm cột PhoneNumber
...
join branch b on b.BranchID = e.BranchID;
select * from branch;
-- 6) Xóa tất cả nhân viên làm việc tại "Chi nhánh Hà Nội" khỏi VIEW EmployeeBranch. Đồng thời hiển thị lại tất cả thông tin của bảng Employees để rút ra nhận xét.
delete from EmployeeBranch
where  EmployeeID = (select EmployeeID from EmployeeBranch where BranchName like "Chi nhánh Hà Nội");
select * from Employees;

-- Không thể xóa dữ liệu trong view 