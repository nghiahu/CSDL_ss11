...
begin
    ...
    update employees set salary = current_salary where employeeid = employee_id;
end //
DELIMITER ;
...
create procedure GetLoanAmountByCustomerID(
	in customer_id int, 
    out total_loan_amount decimal(15,2))
begin
    select coalesce(SUM(loanamount), 0) into total_loan_amount
    from loans
...
/*
4) Viết một Stored Procedure có tên DeleteAccountIfLowBalance. Nhận vào AccountID (IN). 
Nếu số dư nhỏ hơn 1 triệu, xóa tài khoản và thông báo xóa thành công, ngược lại thông báo không thể xóa.
*/
DELIMITER //
...
begin
    ...
    if v_balance < 1000000 then
        ...
        select 'tài khoản đã bị xóa thành công' as message;
    else
...
begin
    ...
    select Balance into ReceiverBalance from Accounts where AccountID = ReceiverAccountID;
...
-- Hãy gọi Stored Procedure UpdateSalaryByID để cập nhật lương của nhân viên có EmployeeID = 4, với mức lương ban đầu là 18 triệu. Sau khi cập nhật, hiển thị mức lương mới.
-- Gọi UpdateSalaryByID
set @CurrentSalary = 18000000;
call UpdateSalaryByID(4, @CurrentSalary);
